...
- .bold[p]`norm`: función de densidad acumuluda 
...
Por ejemplo, supongamos que $X_{i}$ es la variable que resulta de tirar una moneda "justa". Participamos en un concurso que consiste en tirar la misma moneda 100 veces. El premio (G) es $ $1000$ de base más $ $10$ por cada "cara" obtenida ( $X_{i}=1$ ).
...
premios <- replicate(n=10^5,concurso() ); head(premios)
```
...
Ejemplo, retomando CASEN 2007, variable `y = log(ytotcor)`:
...
.pull-left[
Media y desviación estándar en muestra:
]
...
se = stats["sd"]/sqrt(length(data_casen_csv$y)); se 
...
.pull-left[
Inferencia via simulación en `R`: 

]

.pull-right[
[click aquí](https://github.com/mebucca/dar_soc4001/blob/master/slides/class_14/class_14.Rmd)
]
